...
    const [isLoading, setIsLoading] = useState(false);
    // New state to manage the view: Login or Register
...
    };
    ...
    const handleRegister = async (name: string, email: string, password: string, role: UserRole) => {
        ...
        try {
            ...
            // Check if the error message is a string before setting
            setAuthError(error.message);
        } finally {
            setIsLoading(false);
        }
...
        setCurrentUserRole(null);
        setIsRegistering(false); // Reset to default login view
    };
    
...
    if (!currentUser || !currentUserRole) {
        ...
        // Render the Login page (default view)
        return (
            <LoginPage 
                onLogin={handleLogin} 
                error={authError} 
...
                />;
            default:
                // If user is logged in but role is unknown, fall back to default
